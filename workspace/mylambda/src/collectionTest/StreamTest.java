...
//		IntStream.rangeClosed(1, 10).filter(n -> streamTest.check(0)).forEach(System.out::println);
//		IntStream.rangeClosed(1, 10).filter(n ::check).forEach(System.out::println);
...
//				(ArrayList<Integer>) datas.stream().collect(Collectors.toList());
	   
		ArrayList<Integer> times = new ArrayList<Integer>(Arrays.asList(10, 3, 2));
//		map(클래스::static) static일 때는 클래스로 접근한다.
